...
sensitivities = [0.3]*wakeword_length
    ...
    def assist(self):
        ...
        for resp in self.assistant.Assist(iter_log_assist_requests(),
            ...
            if resp.event_type == END_OF_UTTERANCE:
                #print(colored('[ViPi_0.2] + [Sẵn sàng chờ lệnh] ','red'))
            ...
            if resp.speech_results:
                ...
                if "stability: 1.0" in usercommand.lower():
                    ...
                    # if configuration['YouTube']['YouTube_Control']=='Enabled':
                        # if (custom_action_keyword['Keywords']['Phat_mot_bai'][2]).lower() in str(usrcmd).lower():
                            # zingmp3(str(usrcmd).lower())
                            # return continue_conversation
                    #Phát nhạc youtube
                    if configuration['YouTube']['YouTube_Control']=='Enabled':
                        if (custom_action_keyword['Keywords']['YouTube_music_stream'][0]).lower() in str(usrcmd).lower():
                            vlcplayer.stop_vlc()
                            if (custom_action_keyword['Keywords']['Phat_mot_bai'][0]).lower() in str(usrcmd).lower():
                                YouTube_No_Autoplay(str(usrcmd).lower())
                                return continue_conversation
                            if (custom_action_keyword['Keywords']['Phat_mot_bai'][1]).lower() in str(usrcmd).lower():
                                YouTube_No_Autoplay(str(usrcmd).lower())
                                return continue_conversation
                            else:
                    ...
                    if (custom_action_keyword['Keywords']['VLC_music_volume'][0]).lower() in str(usrcmd).lower():
                        if vlcplayer.is_vlc_playing()==True or checkvlcpaused()==True:
                            if (custom_action_keyword['Dict']['Set']).lower() in str(usrcmd).lower() or custom_action_keyword['Dict']['Change'].lower() in str(usrcmd).lower():
                                if '100'.lower() in str(usrcmd).lower() or custom_action_keyword['Dict']['Maximum'] in str(usrcmd).lower():
                                    settingvollevel=100
                                    with open('{}/.mediavolume.json'.format(USER_PATH), 'w') as vol:
                                        json.dump(settingvollevel, vol)
                                elif 'không'.lower() in str(usrcmd).lower() or custom_action_keyword['Dict']['Minimum'] in str(usrcmd).lower():
                                    settingvollevel=0
                                    with open('{}/.mediavolume.json'.format(USER_PATH), 'w') as vol:
                                        json.dump(settingvollevel, vol)
                                else:
                                    for settingvollevel in re.findall(r"[-+]?\d*\.\d+|\d+", str(usrcmd)):
                                        with open('{}/.mediavolume.json'.format(USER_PATH), 'w') as vol:
                                            json.dump(settingvollevel, vol)
                                print('ViPi_0.2 cài đặt volume mức: '+str(settingvollevel))
                            ...
                            say("Sorry nothing is playing right now")
                        return continue_conversation
                    if (custom_action_keyword['Keywords']['Am_lich'][0]).lower() in str(usrcmd).lower():
                        licham(str(usrcmd).lower())
                        return continue_conversation
...
def main(api_endpoint, credentials, project_id,
    ...
    with SampleAssistant(lang, device_model_id, device_id,
        ...
        while True:
            ctr_led('off')
            print(colored('================================','green'))
            print(colored('[ViPi_0.2] + [Sẵn sàng chờ lệnh] ','green'))
