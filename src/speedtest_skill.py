...
def shell():
            ...
            for server in servers:
                ...
                try:
                    ...
                    if e.errno != errno.EPIPE:
                        raise
        sys.exit(0)
    # speak('Đường truyền đang sử dụng mạng của ' % speedtest.config['client'])
    speak('Đang kiểm tra kết nối tới nhà mạng %(isp)s ' % speedtest.config['client'])
    # c = ('Đang kiểm tra thông tin từ sever %(isp)s (%(ip)s)...' % speedtest.config['client'],
            # quiet)
    ...
    # d=('Kết nối đến hostpot %(sponsor)s (%(name)s) [%(d)0.2f km]: '
    ...
    if args.download:
        ...
        # printer('Download: %0.2f M%s/s' %
                # ((results.download / 1000.0 / 1000.0) / args.units[1],
                 # args.units[0]),
                # quiet)
        a = ('Tốc độ Download hiện tại là: %0.2f M%s/s' %
                ((results.download / 1000.0 / 1000.0 )/ args.units[1],
                 args.units[0]))
                # quiet)
# #        a = a.replace("False", "")
    ...
    if args.upload:
        ...
        # printer('Upload: %0.2f M%s/s' %
                # ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 # args.units[0]),
                # quiet)
        b = (' và tốc độ Upload là : %0.2f M%s/s' %
                ((results.upload / 1000.0 / 1000.0) / args.units[1],
                 args.units[0]))
                # quiet)
#        a = a.replace("False", "")
...
def main(data):
    print('[BOT]: XỬ LÝ CÂU LỆNH KIỂM TRA ĐƯỜNG TRUYỀN: '+data)
    try:
        shell()
